...
kmeans_figure <- function(map_file = "output/cyano_16s_kmean_map.Rdata",
                       reg_file = "output/cyano_16s_kmean_glm.Rdata",
                       figure_name = paste0("figures/cyano_16s_kmeans_",Sys.Date(),".pdf"),
                       som_color_vector = c("darkred", "darkblue"),
                       som_name_vector = c("Nearshore", "Offshore"),
                       centroid_color = c("blue","red"), switch = TRUE,
  ...
  if(switch == TRUE){Letter_1 <- "B. "}else{Letter_1 <- "A. "}
  ...
  if(switch == TRUE){Letter_2 <- "A. "}else{Letter_2 <- "B. "}
  ...
  summary_fit <- summary(glm(som_maps$som_1~som_maps$NC_mean, family = "binomial"))
  
  p_val <- summary_fit$coefficients[2,4]
  
  summary_fit <- summary(glm(som_maps$som_1~som_maps$Dist_mean, family = "binomial"))
  
  p_val <- summary_fit$coefficients[2,4]
  
  nc_plot <- ggplot(som_plots, aes(x = NC_mean, y = freq, color = cluster)) +
    geom_point(pch = 20, size = 4) +
    theme(legend.title = element_blank(),
    ...
    stat_smooth(method = "glm", level = 0.95, method.args = list(family = "binomial")) +
    scale_color_manual(values = c("red", "blue")) +
    xlab("Mean Nutricline Depth (m)") + ylab("Frequency") +
    ggtitle("C. SOM Freq ~ Mean Nutricline Depth (m)")
  
  leg <- get_legend(nc_plot)
  
  nc_plot <- nc_plot + theme(legend.position = "none")
  
  dist_plot <- ggplot(som_plots, aes(x = Dist_mean, y = freq, color = cluster)) +
    geom_point(pch = 20, size = 4) +
    theme(legend.title = element_blank(),
          panel.border = element_rect(fill = NA, color = "black", linetype = "solid"),
          panel.background = element_blank(),
    ...
    stat_smooth(method = "glm", level = 0.95, method.args = list(family = "binomial")) +
    scale_color_manual(values = c("red", "blue")) +
    xlab("Distance to Coast (km)") + ylab("Frequency") +
    ggtitle("D. SOM Freq ~ Distance to Coast (km)")
  
  temp_som_plots <- plot_grid(plot_grid(nc_plot, dist_plot, ncol = 2),
  ...
  if(switch == TRUE){
    ...
    a <- plot_grid(title,  
                   plot_grid(p2,p1, ncol = 2),
                   temp_som_plots, ncol = 1, nrow = 3, rel_heights = c(0.1,1,1))
    print(a)
    dev.off()
  }
  if(switch != TRUE){
...
full_aic_table_figure <- function(in_plastid = "output/plastid_16s_map.Rdata",
  ...
  phyto_AIC <- phyto_AIC[c(1,3,4,5,6,20,9,10,12,13,14,15,21,18,19),]
  bacteria_AIC <- bacteria_AIC[c(1,3,4,5,6,20,9,10,12,13,14,15,21,18,19),]
  eukaryota_AIC <- eukaryota_AIC[c(1,3,4,5,6,20,9,10,12,13,14,15,21,18,19),]
  cyano_AIC <- cyano_AIC[c(1,3,4,5,6,20,9,10,12,13,14,15,21,18,19),]
...
full_aic_table_figure_diversity <- function(
                                  ...
                                  figure_name_2 = paste0("figures/full_aic_plot_logit_even_",Sys.Date(),".pdf"),
  ...
  # col = 2 evenness col = 3 shannon col = 4 richness
  
  cyano_AIC <- aic_table_func_diveristy(som_maps = cyano_plots, i = col)
  cyano_AIC <- as.data.frame(cyano_AIC, stringsAsFactors = FALSE)
  colnames(cyano_AIC) <- c("Variables","AIC")
  cyano_AIC$AIC <- as.numeric(cyano_AIC$AIC)
  cyano_AIC$AIC <- round(cyano_AIC$AIC, digits = 2)
  
  bacteria_AIC <- aic_table_func_diveristy(som_maps = bacteria_plots, i = col)
  bacteria_AIC <- as.data.frame(bacteria_AIC, stringsAsFactors = FALSE)
  colnames(bacteria_AIC) <- c("Variables","AIC")
  bacteria_AIC$AIC <- as.numeric(bacteria_AIC$AIC)
  bacteria_AIC$AIC <- round(bacteria_AIC$AIC, digits = 2)
  
  eukaryota_AIC <- aic_table_func_diveristy(som_maps = eukaryota_plots, i = col)
  eukaryota_AIC <- as.data.frame(eukaryota_AIC, stringsAsFactors = FALSE)
  colnames(eukaryota_AIC) <- c("Variables","AIC")
  eukaryota_AIC$AIC <- as.numeric(eukaryota_AIC$AIC)
  eukaryota_AIC$AIC <- round(eukaryota_AIC$AIC, digits = 2)
  
  phyto_AIC  <- aic_table_func_diveristy(som_maps = phyto_plots, i = col)
  phyto_AIC <- as.data.frame(phyto_AIC, stringsAsFactors = FALSE)
  colnames(phyto_AIC) <- c("Variables","AIC")
  phyto_AIC$AIC <- as.numeric(phyto_AIC$AIC)
  phyto_AIC$AIC <- round(phyto_AIC$AIC, digits = 2)
...
full_aic_table_figure_kmeans <- function(in_cyano = "output/cyano_16s_kmean_map.Rdata",
                                  in_bacteria = "output/bact_m_euks_16s_kmean_map.Rdata",
                                  in_euks = "output/euks_hetero_18sv9_kmean_map.Rdata",
                                  in_phyto = "output/euks_auto_18sv9_kmean_map.Rdata", minimum_tp = 8,
                                  figure_name = paste0("figures/full_aic_table_logit_kmeans_",Sys.Date(),".pdf"),
                                  figure_name_2 = paste0("figures/full_aic_plot_logit_kmeans_",Sys.Date(),".pdf"),
                                  title_name = "Variable Importance"){
  # load an rename data
  ...
  pdf(figure_name_2, width = 8, height = 6)
  print(ggplot(data = plot_df, aes(x = Group, y = Variables, size = AIC)) + 
          geom_point(fill = "red", color = "black", alpha = 0.6, shape = 21) +
          labs(size = "Variable\n Importance") + ylab("Variable") +
          theme(panel.background = element_blank(),
...
diversity_stacked_figure <- function(in_cyano = "output/cyano_16s_map.Rdata",
                                     ...
                                     in_euks = "output/euks_hetero_18sv9_map.Rdata",
                                     in_phyto = "output/euks_auto_18sv9_map.Rdata",
                                     figure_name = paste0("figures/richness_evenness_summary_",Sys.Date(),".pdf"),
                                     figure_name2 =  paste0("figures/diversity_summary_",Sys.Date(),".pdf")){
  ...
  cyano_shannon <- ggplot() + 
    ...
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "shannon"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 1.25) +
    ggtitle(paste0("Shannon Diversity")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  cyano_evenness <- ggplot() + 
    ...
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "evenness"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient(low = "white", high = "red", aes(min = min(evenness, na.rm = TRUE, max = max(evenness, na.rm = TRUE)), limits = c(min+0.02,max-0.02))) +
    ggtitle(paste0("Evenness")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  title <- ggdraw() + draw_label("Cyanobacteria", fontface='bold')
  cyano <- plot_grid(title, plot_grid(cyano_richness, cyano_evenness, ncol = 2), nrow = 1, rel_widths = c(0.2,1))
  
  cyano_sha <- plot_grid(title, cyano_shannon, nrow = 2, rel_heights = c(0.12,1))
  ...
  bacteria_shannon <- ggplot() + 
    ...
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "shannon"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 4.5) +
    ggtitle(paste0("Shannon Diversity")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  bacteria_richness <- ggplot() + 
    geom_polygon(data = map, aes(x=long, y = lat, group = group), fill = "grey", color = "black") + 
    coord_fixed(xlim = c(-125.3206, -116.3055),ylim= c(28.84998,38.08734), 1.3) +
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "richness"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient(low = "white", high = "blue", aes(min = min(richness, na.rm = TRUE, max = max(richness, na.rm = TRUE)), limits = c(min,max))) +
    ggtitle(paste0("Richness")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  bacteria <- plot_grid(title, plot_grid(bacteria_richness, bacteria_evenness, ncol = 2), nrow = 1, rel_widths = c(0.2,1))
  
  bact_sha <- plot_grid(title, bacteria_shannon, nrow = 2, rel_heights = c(0.12,1))
  ...
  phyto_shannon <- ggplot() + 
    ...
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "shannon"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 3.6) +
    ggtitle(paste0("Shannon Diversity")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  title <- ggdraw() + draw_label("Eukaryotic\nPhytoplankton", fontface='bold')
  phyto <- plot_grid(title, plot_grid(phyto_richness, phyto_evenness, ncol = 2), nrow = 1, rel_widths = c(0.2,1))
  
  phyto_sha <- plot_grid(title, phyto_shannon, nrow = 2, rel_heights = c(0.12,1))
  ...
  euks_shannon <- ggplot() + 
    ...
    xlab("Longitude") + ylab("Latitude") + 
    geom_point(data = som_maps, aes_string(x = "long", y = "lat", fill = "shannon"), color = "black", size =5, stroke = 0.1, shape = 21) +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 3.5) +
    ggtitle(paste0("Shannon Diversity")) +
    theme(legend.title = element_blank(),
          panel.background = element_blank(),
          panel.border = element_rect(fill = NA),
  ...
  title <- ggdraw() + draw_label("Heterotrophic\nEukaryotes", fontface='bold')
  euks <- plot_grid(title, plot_grid(euks_richness, euks_evenness, ncol = 2), nrow = 1, rel_widths = c(0.2,1))
  
  euks_sha <- plot_grid(title, euks_shannon, nrow = 2, rel_heights = c(0.12,1))
  ...
  div <- plot_grid(phyto_sha, euks_sha, cyano_sha, bact_sha)
  
  pdf(figure_name2, width = 6, height = 7)
  print(div)
  dev.off()
...
som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/flavo_16s_map.Rdata",
           reg_file = "output/flavo_16s_glm.Rdata",
           figure_name = paste0("figures/flavo_16s_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkred", "darkblue"),
           som_name_vector = c("Nearshore", "Offshore"),
           centroid_color = c("blue","red"), switch = TRUE,
           main = "16s Flavobacteria", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/rhodo_16s_map.Rdata",
           reg_file = "output/rhodo_16s_glm.Rdata",
           figure_name = paste0("figures/rhodo_16s_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkred", "darkblue"),
           som_name_vector = c("Nearshore", "Offshore"),
           centroid_color = c("blue","red"), switch = TRUE,
           main = "16s Rhodobacterales", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/sar_16s_map.Rdata",
           reg_file = "output/sar_16s_glm.Rdata",
           figure_name = paste0("figures/sar_16s_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkred", "darkblue"),
           som_name_vector = c("Nearshore", "Offshore"),
           centroid_color = c("blue","red"), switch = TRUE,
           main = "16s SAR Clade", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/archaea_16s_map.Rdata",
           reg_file = "output/archaea_16s_glm.Rdata",
           figure_name = paste0("figures/archaea_16s_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkblue", "darkred"),
           som_name_vector = c("Offshore", "Nearshore"),
           centroid_color = c("red","blue"), switch = FALSE,
           main = "16s Archaea", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/diatom_18sv9_map.Rdata",
           reg_file = "output/diatom_18sv9_glm.Rdata",
           figure_name = paste0("figures/diatom_18sv9_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkblue", "darkred"),
           som_name_vector = c("Offshore", "Nearshore"),
           centroid_color = c("red","blue"), switch = FALSE,
           main = "18sv9 Diatom", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/dino_18sv9_map.Rdata",
           reg_file = "output/dino_18sv9_glm.Rdata",
           figure_name = paste0("figures/dino_18sv9_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkblue", "darkred"),
           som_name_vector = c("Offshore", "Nearshore"),
           centroid_color = c("red","blue"), switch = FALSE,
           main = "18sv9 Dinoflagellates", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/syndin_18sv9_map.Rdata",
           reg_file = "output/syndin_18sv9_glm.Rdata",
           figure_name = paste0("figures/syndin_18sv9_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkblue", "darkred"),
           som_name_vector = c("Offshore", "Nearshore"),
           centroid_color = c("red","blue"), switch = FALSE,
           main = "18sv9 Syndiniales", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/hapto_18sv9_map.Rdata",
           reg_file = "output/hapto_18sv9_glm.Rdata",
           figure_name = paste0("figures/hapto_18sv9_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkred", "darkblue"),
           som_name_vector = c("Nearshore", "Offshore"),
           centroid_color = c("blue","red"), switch = TRUE,
           main = "18sv9 Haptophytes", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))

som_figure(SST_file = "output/CALCOFI_temp_tables.Rdata",
           map_file = "output/metazoa_18sv9_map.Rdata",
           reg_file = "output/metazoa_18sv9_glm.Rdata",
           figure_name = paste0("figures/metazoa_18sv9_som_",Sys.Date(),".pdf"),
           som_color_vector = c("darkred", "darkblue"),
           som_name_vector = c("Nearshore", "Offshore"),
           centroid_color = c("blue","red"), switch = TRUE,
           main = "18sv9 Metazoans", mean_position = c(15,1.2),
           coeff_position = c(0.123,1.2))
...
kmeans_figure(map_file = "output/cyano_16s_kmean_map.Rdata",
              reg_file = "output/cyano_16s_kmean_glm.Rdata",
              figure_name = paste0("figures/cyano_16s_kmeans_",Sys.Date(),".pdf"),
              som_color_vector = c("darkred", "darkblue"),
              som_name_vector = c("Nearshore", "Offshore"),
...
kmeans_figure(map_file = "output/bact_m_euks_16s_kmean_map.Rdata",
              reg_file = "output/bact_m_euks_16s_kmean_glm.Rdata",
              figure_name = paste0("figures/bact_m_euks_16s_kmeans_",Sys.Date(),".pdf"),
              som_color_vector = c("darkblue", "darkred"),
              som_name_vector = c("Offshore", "Nearshore"),
              centroid_color = c("red","blue"), switch = FALSE,
              main = "16s Heterotrophic Bacteria/Archea")

kmeans_figure(map_file = "output/euks_auto_18sv9_kmean_map.Rdata",
              reg_file = "output/euks_auto_18sv9_kmean_glm.Rdata",
              figure_name = paste0("figures/euks_auto_18sv9_kmeans_",Sys.Date(),".pdf"),
              som_color_vector = c("darkred", "darkblue"),
              som_name_vector = c("Nearshore", "Offshore"),
              centroid_color = c("blue","red"), switch = TRUE,
              main = "18sv9 Eukaryotic Phytoplankton")

kmeans_figure(map_file = "output/euks_hetero_18sv9_kmean_map.Rdata",
              reg_file = "output/euks_hetero_18sv9_kmean_glm.Rdata",
              figure_name = paste0("figures/euks_hetero_18sv9_kmeans_",Sys.Date(),".pdf"),
              som_color_vector = c("darkblue", "darkred"),
              som_name_vector = c("Offshore", "Nearshore"),
              centroid_color = c("red","blue"), switch = FALSE,
              main = "18sv9 Heterotrophic Eukaryotes")
