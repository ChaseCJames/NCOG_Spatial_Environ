...
sixteen_s$som_id <- som_ids

full_dat$som_id <- sixteen_s$som_id[match(full_dat$eco_name, rownames(sixteen_s))]

# diversity evenness

sixteen_s$shannon_index <- diversity(six_copy, MARGIN = 1, index = "shannon")
full_dat$shannon <- sixteen_s$shannon_index[match(full_dat$eco_name, rownames(sixteen_s))]

S <- apply(six_copy>0,1,sum)
sixteen_s$evenness <- diversity(six_copy, index="shannon")/log(S)
full_dat$evenness <- sixteen_s$evenness[match(full_dat$eco_name, rownames(sixteen_s))]

# richness

richness <- apply(six_copy, 1, function(x) length(which(x != 0)))

sixteen_s$richness <- richness
full_dat$richness <- sixteen_s$richness[match(full_dat$eco_name, rownames(sixteen_s))]

# Dissimilarity

dissimilar <- vegdist(six_copy, method = "jaccard", binary = TRUE)
...
mean_som$shannon_index <- NULL
mean_som$evenness <- NULL
...
mean_som$groups <- split_taxa$D[match(mean_som$ASV, split_taxa$Feature.ID)]

ggplot(mean_som, aes(x = som_id, y = mean_rel_abun, fill = groups)) + geom_col() + 
...
mean_plot <- ggplot(som_plots, aes(x = temp_mean, y = freq, color = cluster)) +
  ...
  annotate("text", x = 15, y = 0.5, label = paste0("R Squared = ", round(r_sq_mean, digits = 3)), size = 3) + ggtitle("F. SOM Freq ~ Mean SST")
...
coeff_plot <- ggplot(som_plots, aes(x = temp_coeff, y = freq, color = cluster)) +
  ...
  annotate("text", x = 0.123, y = 0.5, label = paste0("R Squared = ", round(r_sq_coeff, digits = 3)), size = 3) + ggtitle("E. SOM Freq ~ Coeff. Var. SST")

temp_som_plots <- plot_grid(coeff_plot, mean_plot, ncol = 2,rel_widths = c(0.77,1))
...
mean_plot <- ggplot(som_plots, aes(x = temp_mean, y = shannon)) +
  ...
  annotate("text", x = 15, y = 4.73, label = paste0("R Squared = ", round(r_sq_mean, digits = 3)), size = 4)
...
coeff_plot <- ggplot(som_plots, aes(x = temp_coeff, y = shannon)) +
  ...
  annotate("text", x = 0.13, y = 4.73, label = paste0("R Squared = ", round(r_sq_coeff, digits = 3)), size = 4)
...
mean_plot <- ggplot(som_plots, aes(x = temp_mean, y = evenness)) +
  ...
  annotate("text", x = 15.4, y = 0.71, label = paste0("R Squared = ", round(r_sq_mean, digits = 3)), size = 4)
...
coeff_plot <- ggplot(som_plots, aes(x = temp_coeff, y = evenness)) +
  ...
  annotate("text", x = 0.1, y = 0.715, label = paste0("R Squared = ", round(r_sq_coeff, digits = 3)), size = 4)
...
mean_plot <- ggplot(som_plots, aes(x = temp_mean, y = richness)) +
  ...
  annotate("text", x = 15, y = 900, label = paste0("R Squared = ", round(r_sq_mean, digits = 3)), size = 4)
...
coeff_plot <- ggplot(som_plots, aes(x = temp_coeff, y = richness)) +
  ...
  annotate("text", x = 0.125, y = 900, label = paste0("R Squared = ", round(r_sq_coeff, digits = 3)), size = 4)
...
regsubsetsObj$xnames <- c("(Intercept)", "Mean P04", "Mean NO3", "Mean SiO3","Mean Temp",
                          "Coeff. Var. PO4", "Coeff. Var. NO3", "Coeff. Var. SiO3","Coeff. Var. Temp")

pdf(file = "figures/16s_stepwise_regression_leaps_0723.pdf", width = 6, height = 5)
...
glm_simple <- glm(som_1 ~ temp_mean + NO3_coeff + temp_coeff, data = som_glm)

simple_sum <- summary(glm_simple)
model_AIC[10,2] <- simple_sum$aic
model_AIC[10,1] <- "Mean Temp +\n Coeff Var. NO3 + Coeff Var. Temp"

AIC_table <- model_AIC[order(model_AIC$AIC, decreasing = TRUE),]

AIC_table$AIC <- round(AIC_table$AIC, 3)

pdf(file = "figures/16s_AIC_table_0723.pdf", height = 5, width = 7)
...
varImpPlot(forest_out, type = 2, main = paste0("16s Random Forest"))

pdf(file = "figures/16s_random_forest_importance_0723.pdf", width = 5, height = 4)
varImpPlot(forest_out, type = 2, main = paste0("16s Random Forest"))
dev.off()

# Random Forest for Diversity

forest_div <- full_dat[,-which(is.na(rowSums(full_dat[,c(32,33,37,38,39,60)])))]

train <- sample(nrow(forest_div), 0.7*nrow(forest_div), replace = FALSE)

forest_out <- randomForest(shannon ~  CC_Depth + T_degC + PO4ug + SiO3ug +
                             NO3ug, data = forest_div, importance = TRUE,
                           na.action = na.omit, subset = train, mtry = 4, ntree = 1000)


# Predicting on validation set
predTrain <- predict(forest_out, forest_div[train,])
# Checking classification accuracy
table(predTrain, forest_div$shannon[train]) 

# Predicting on validation set
predValid <- predict(forest_out, forest_div[-train,])
# Checking classification accuracy
table(predValid, forest_div$shannon[-train])  

varImpPlot(forest_out, type = 1, main = paste0("16s Random Forest"))

mean(abs(predValid-forest_div$shannon[-train]),na.rm = TRUE)
...
shannon_depth_dist <- ggplot(full_dat, aes(x = dist_to_coast, y = CC_Depth, fill = shannon)) + 
  ...
  scale_fill_viridis(name = "Shannon Diversity", option = "D", limits = c(3.8,5.4), oob = scales::squish) +
  ylab("Depth (m)") + xlab("Distance to Coast (km)") 

even_depth_dist <- ggplot(full_dat, aes(x = dist_to_coast, y = CC_Depth, fill = evenness)) + 
  geom_point(size = 4, pch = 21, color = "black") + ylim(150,0) + 
  scale_fill_viridis(name = "Evenness", option = "D", limits = c(0.6,0.8), oob = scales::squish) +
  ylab("Depth (m)") + xlab("Distance to Coast (km)") 
...
shannon_timeplot <- ggplot(full_dat, aes(x = Date, y = dist_to_coast, fill = shannon)) + 
  ...
  scale_fill_viridis(name = "Shannon", option = "D", limits = c(3.8,5.4), oob = scales::squish) +
...
title <- ggdraw() + draw_label("All 16s", fontface='bold')
theme_set(theme_cowplot(font_size=8))

pdf(file = "figures/all16s_som_summary_0717.pdf", width = 8, height = 10)
plot_grid(title, plot_grid(sst,sst_mean, ncol = 2), 
          plot_grid(p1,p2, ncol = 2),
          temp_som_plots, ncol = 1, nrow = 4, rel_heights = c(0.1,1,1,1))
dev.off()

title1 <- ggdraw() + draw_label("All 16s Diversity/Evenness/Richness", fontface='bold')
pdf(file = "figures/all16s_diversity_summary_0717.pdf", width = 12, height = 12)
...
title2 <- ggdraw() + draw_label("All 16s Diversity/Evenness/Richness over Time", fontface='bold')
pdf(file = "figures/all16s_diversity_time_0717.pdf", width = 8, height = 12)
...
title_dissim <- ggdraw() + draw_label("16s Dissimilarity Plots", fontface='bold')

theme_set(theme_cowplot(font_size=8))

pdf(file = "figures/16s_dissimilarity_0807.pdf", width = 12, height = 6)
...
pdf(file = "figures/16s_temp_dissim_0722.pdf", width = 5, height = 5)
temp_dissimilar
dev.off()

pdf(file = "figures/16s_chl_dissim_0722.pdf", width = 5, height = 5)
chl_dissimilar
dev.off()

pdf(file = "figures/16s_n03_dissim_0722.pdf", width = 5, height = 5)
n03_dissimilar
dev.off()

pdf(file = "figures/16s_depth_dissim_0722.pdf", width = 5, height = 5)
depth_dissimilar
dev.off()

pdf(file = "figures/16s_date_dissim_0722.pdf", width = 5, height = 5)
date_dissimilar
dev.off()

pdf(file = "figures/16s_season_dissim_0722.pdf", width = 5, height = 5)
season_dissimilar
dev.off()
...
six_sums$station <- full_dat$Sta_ID[match(rownames(six_copy), full_dat$eco_name)]

station_sums <- six_sums %>% 
  group_by(station) %>% 
...
title_stat <- ggdraw() + draw_label("16s Diversity/Evenness/Richness by Station over Time", fontface='bold')

theme_set(theme_cowplot(font_size=8))

pdf(file = "figures/16s_station_div_0722.pdf", width = 10, height = 6)
...
div_plot <- ggplot(som_maps, aes(x = temp_coeff)) + 
  ...
  theme(legend.title = element_blank(),
        ...
        panel.background = element_blank()) +
  ggtitle("16s Mean Alpha Diversity vs Gamma Diversity per Station") +
  scale_color_manual(values = c("royalblue2", "seagreen3")) +
  annotate("text", x = 0.097, y = 4.6, label = paste0("R Squared = ", round(alpha_r_sq_coeff, digits = 3)), size = 4) +
  annotate("text", x = 0.125, y = 5.9, label = paste0("R Squared = ", round(gamma_r_sq_coeff, digits = 3)), size = 4)

theme_set(theme_cowplot(font_size=10))

pdf(file = "figures/16s_alpha_v_gamma_0723.pdf", width = 8, height = 6)
div_plot
dev.off()
